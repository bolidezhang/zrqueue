...
## Overview
`zrqueue::SPSCQueue` is a wait-free, lock-free, fixed-size single-producer single-consumer queue provided as a **single-header** C++ library. It is proven to be significantly faster and more stable than `rigtorp::SPSCQueue`. 

Designed strictly for High-Frequency Trading (HFT) and ultra-low latency (ULL) systems where every nanosecond counts, `zrqueue` applies hardware-level micro-architectural optimizations including zero-branching, strict cache-line isolation, OS HugePages support, and hardware pausing. 
...
* **Static In-Place Array (`SPSCStaticQueue`):** An alternative zero-allocation queue where data is embedded directly within the object footprint, eliminating the final pointer-chasing (dereference) penalty.
...
| **Pointer Dereference** | Always `slots_[idx]` (Heap) | `SPSCStaticQueue` (In-place) | Faster base-offset addressing, zero dynamic allocation. |
...
static zrqueue::SPSCStaticQueue<Tick, 4096> g_hot_queue;
...
Requirements:
C++ Standard: C++11/C++14 minimum. (Automatically utilizes C++17 optimizations if available).
